...
import {
  ...
  LayoutAnimation, Platform, UIManager, Modal, TextInput, Alert,
} from 'react-native';
...
import { fetchPropertyDetails, addSpace, addAsset, addHistory } from '../../services/propertyDetails';
...
import type { SpaceWithAssets, AssetWithChangelog, EditableSpec } from '../../types';

if (Platform.OS === 'android' && UIManager.setLayoutAnimationEnabledExperimental) {
  UIManager.setLayoutAnimationEnabledExperimental(true);
}

const FormModal = ({ visible, onClose, title, children, onSubmit, submitText = "Add" }: any) => (
    <Modal animationType="slide" transparent={true} visible={visible} onRequestClose={onClose}>
        <View style={styles.modalOverlay}><View style={styles.modalContainer}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>{title}</Text><TouchableOpacity onPress={onClose} style={styles.closeButton}><X size={24} color={PALETTE.textSecondary} /></TouchableOpacity></View>
            <ScrollView keyboardShouldPersistTaps="handled">{children}</ScrollView>
            <TouchableOpacity style={styles.submitButton} onPress={onSubmit}><Text style={styles.submitButtonText}>{submitText}</Text></TouchableOpacity>
        </View></View>
    </Modal>
);

const SpecificationDetails = ({ specifications }: { specifications: Record<string, any> }) => (
  <View style={styles.specificationsBox}>{Object.entries(specifications).map(([key, value]) => (
      <View key={key} style={styles.specPair}><Text style={styles.specKey}>{key.replace(/_/g, ' ')}</Text><Text style={styles.specValue}>{String(value)}</Text></View>
  ))}</View>
);

const AssetAccordion = ({ asset, isExpanded, onToggle, onAddHistory }: { asset: AssetWithChangelog, isExpanded: boolean, onToggle: () => void, onAddHistory: (asset: AssetWithChangelog) => void }) => {
    ...
    <View style={styles.assetContainer}>
      ...
      {isExpanded && (
        <View style={styles.assetContent}>
          ...
          {latestChange ? <SpecificationDetails specifications={latestChange.specifications} /> : <Text style={styles.emptyText}>No accepted specifications yet.</Text>}
          <View style={styles.historySectionHeader}>
            <Text style={styles.contentSectionTitle}>History</Text>
            <TouchableOpacity style={styles.addButtonSmall} onPress={() => onAddHistory(asset)}><PlusCircle size={18} color={PALETTE.primary} /><Text style={styles.addButtonSmallText}>Add Entry</Text></TouchableOpacity>
          </View>
          {acceptedLogs.length > 1 ? (
            acceptedLogs.slice(1).map((entry) => (
              <View key={entry.id} style={styles.historyItemContainer}>
                  <TouchableOpacity style={styles.historyEntry} onPress={() => setExpandedHistoryId(prev => prev === entry.id ? null : entry.id)}>
                    <View style={styles.historyHeader}><Text style={styles.historyDate}>{new Date(entry.created_at).toLocaleString()}</Text>{expandedHistoryId === entry.id ? <ChevronDown color={PALETTE.textSecondary} size={16} /> : <ChevronRight color={PALETTE.textSecondary} size={16} />}</View>
                  ...
                  {expandedHistoryId === entry.id && <View style={styles.historySpecBox}><SpecificationDetails specifications={entry.specifications} /></View>}
              </View>
            ))
...
const PropertyDetails = ({ route, navigation }: { route: any; navigation: any }) => {
  ...
  const handleAddSpace = async () => {
    ...
    try {
      ...
      setNewSpaceName(''); setNewSpaceType(null); setAddSpaceModalVisible(false);
  ...
  const handleAddAsset = async () => {
    ...
    try {
      ...
      setNewAssetDescription(''); setSelectedAssetType(null); setAddAssetModalVisible(false);
    } catch (error: any) { Alert.alert("Error", error.message); }
  };
  ...
  const handleAddHistory = async () => {
    ...
    try {
      ...
      setNewHistoryDescription(''); setAddHistoryModalVisible(false); setCurrentAsset(null);
    } catch (error: any) { Alert.alert("Error", error.message); }
  };
  ...
  const openAddHistoryModal = (asset: AssetWithChangelog) => {
    ...
    const specsArray = Object.entries(latestSpecs).map(([key, value], index) => ({ id: index, key: value as string, value: value as string }));
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.dropdownContainer}>
          <DropField options={spaces.map(s => s.name)} selectedValue={selectedSpaceName} onSelect={(name) => {
        ...
        <View style={styles.contentContainer}>
          {currentAssets.length > 0 ? (
            currentAssets.map(asset => (
              <AssetAccordion key={asset.id} asset={asset} isExpanded={expandedAssetId === asset.id}
