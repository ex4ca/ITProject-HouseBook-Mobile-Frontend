...
import { fetchMyProfile, fetchPropertyOwner, fetchPropertyTradies } from '../../services/FetchAuthority';
...
const PropertyOwnerCard = ({ owner }: { owner: UserProfile | null }) => {
    ...
    return (
        <View style={styles.section}>
            ...
            <Text style={styles.ownerInfoLine}>{owner.phone || 'No phone number provided'}</Text>
        </View>
    );
};
...
const AuthorityManagementCard = ({ tradies }: { tradies: Tradie[] }) => (
    <View style={styles.section}>
        ...
        {tradies.length > 0 ? tradies.map(tradie => {
            const initials = tradie.name.split(' ').map(n => n[0]).join('');
            return (
                <View key={tradie.id} style={styles.userRow}>
                    <View style={styles.userInfo}>
                        <View style={styles.smallAvatar}><Text>{initials}</Text></View>
                        <View style={styles.userDetails}>
                            <Text style={styles.userRowName}>{tradie.name}</Text>
                            <Text style={styles.userStatus}>{tradie.status.replace('_', ' ')}</Text>
                        </View>
                    </View>
  ...
  if (loading) {
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ActivityIndicator style={{ flex: 1 }} size="large" color={PALETTE.primary} />
        </SafeAreaView>
    );
  }
